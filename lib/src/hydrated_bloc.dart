...
mixin HydratedMixin<State> on BlocBase<State> {
  ...
  void hydrate([Storage? storage]) {
    newStorage = storage ?? HydratedBloc.storage;
    try {
  ...
  State get state {
    ...
    try {
      final stateJson = newStorage.read(storageToken) as Map<dynamic, dynamic>?;
      if (stateJson == null) {
        _state = super.state;
        return super.state;
      }
  ...
  Future<void> clear() => newStorage.delete(storageToken);
